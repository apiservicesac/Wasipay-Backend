...
import { GetByIdUseCase, UpdateImageUseCase } from '@/client/application/use_cases';
...
export const updateImageController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const entity :any = await findEntity.run(id)
        ...
        const datUpdated = await useCase.run(id, images_entities[0]._id!)
